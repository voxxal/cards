...
// https://github.com/Nairou/fontstash-zig
pub const Context = struct {
    ...
    pub fn textBounds(self: *Context, x: f32, y: f32, string: []const u8, bounds: ?*Bounds) f32 {
        ...
        if (bounds) |b| {
            b.minx = boundsArray[0];
            b.miny = boundsArray[1];
            b.maxx = boundsArray[2];
            b.maxx = boundsArray[3];
        }
        return advance;
    }
...
pub extern fn sfons_create([*c]const Desc) *c.FONScontext;
pub fn create(desc: Desc) Context {
    return Context{ .context = sfons_create(&desc) };
}

pub extern fn sfons_destroy(*const c.FONScontext) void;
pub fn destroy(ctx: Context) void {
    sfons_destroy(ctx.context);
}

pub extern fn sfons_flush(*const c.FONScontext) void;
pub fn flush(ctx: Context) void {
    sfons_flush(ctx.context);
}
